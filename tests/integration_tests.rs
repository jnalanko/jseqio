...
fn fastq() {
    ...
    let mut reader = FastXReader::new_with_format(input, FileType::FASTQ);
    ...
    let mut reader2 = FastXReader::new_with_format(written_data.as_slice(), FileType::FASTQ);
...
fn fasta() {
    ...
    let mut reader = FastXReader::new_with_format(input, FileType::FASTA);
    ...
    let mut reader2 = FastXReader::new_with_format(written_data.as_slice(), FileType::FASTA);
    let mut seqs_read2 = 0;
    while let Some(record) = reader2.read_next().unwrap() {
        dbg!(&record);
        assert_eq!(record.head, headers[seqs_read2].as_bytes());
...
fn test_into_db(){
    ...
    let db2 = FastXReader::new_with_format(
...
fn test_empty_file() {
    ...
    let bufreader = BufReader::new(data.as_slice());
    let reader = FastXReader::new_with_format(bufreader, FileType::FASTA);
    let db = reader.into_db().unwrap(); // Should not panic
    assert_eq!(db.iter().count(), 0);

    // non-dynamic FASTQ reader
    let bufreader = BufReader::new(data.as_slice());
    let reader = FastXReader::new_with_format(bufreader, FileType::FASTQ);
    let db = reader.into_db().unwrap(); // Should not panic
    assert_eq!(db.iter().count(), 0);
}
...
fn test_flush_on_drop(){
    ...
    let mut reader = FastXReader::new_with_format(BufReader::new(File::open("/tmp/test.fna").unwrap()), FileType::FASTA);
